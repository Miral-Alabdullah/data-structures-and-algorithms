...
const typeNum = (arr) => {
  let forNumbersOnly = arr.filter(value => {if(typeof(value)==='number') return value;});
  return forNumbersOnly;
};
...
const containsAnd = (arr) => {
  let onlyAndKeyword = arr.filter(value => value.includes('and'));
  return onlyAndKeyword;
};
...
const oddValues = (arr) => {
  let oddIntegersOnly = arr.filter(value => value % 2 !== 0);
  return oddIntegersOnly;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let unCommonElement = arr.filter(value => !forbiddenValues.includes(value));
