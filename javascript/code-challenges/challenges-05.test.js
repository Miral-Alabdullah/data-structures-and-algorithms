...
const toLastNames = people => {
  let listOfPeople = people.map(value => {
...
const addValues = (arr) => {
  let arrayOfSumm = arr.reduce((accumulator, value) =>{
...
const addPurchases = (arr) => {
  let totalAmountPurshased = arr.reduce((accumulator, value) => {
    let total = accumulator + value.purchasePrice;
    return total;
  },0);
  return totalAmountPurshased;
};
...
const returnNames = (arr) => {
  let nameOfTheCharacters = [];
  arr.reduce((accumulator, value) => {
    nameOfTheCharacters.push(value.name);
    return accumulator;
  }, 0);
  return nameOfTheCharacters;
};
