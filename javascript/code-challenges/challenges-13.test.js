...
Write a function named longestString that takes in an array of strings and returns
the index position of the longest string.
------------------------------------------------------------------------------------------------ */

const longestString = (arr) => {
  let longest = '';
  return arr.reduce((acc, val, idnx) => {
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(ele => ele.substring(1, 4) + ele.substring(6, 9) + ele.substring(10, 14));
};
...
const allHappy = (arr) => {
  let happy = arr.map(ele => (ele.includes(':)')) ? true : false);
  return (happy.includes(false)) ? false : true;
};
